...
int main()
{
    ...
    for(int i=0; i<thresholdedSegmentations.size(); i++)
    {
        ...
        accuracyMetricTotal += accuracyMetricForSegmentation(mask, threshSegmentBGR);
        totalHand += pixelPrecisionHand(mask, threshSegmentBGR);
        totalNonHand += pixelPrecisionNonHand(mask, threshSegmentBGR);
        
    }
    
    double pixelsPrecisionHandMean = totalHand/thresholdedSegmentations.size();
    cout.precision(2);
    cout << "Pixel precision class Hand: " << pixelsPrecisionHandMean << endl;
    double pixelsPrecisionNonHandMean = totalNonHand/thresholdedSegmentations.size();
    cout << "Pixel precision class Non Hand: " << pixelsPrecisionNonHandMean << endl;
    
    double pixelsAccuracy = accuracyMetricTotal/thresholdedSegmentations.size();
    cout << "Pixel accuracy class both: " << pixelsAccuracy << endl;
